...
export function pointerPositionToNote(e: PointerEvent, element: HTMLElement): number {
    const rect = element.getBoundingClientRect();
    const x = e.clientX - rect.left;
    // const y = rect.bottom - e.clientY;
    const noteRange = 88; // Piano range
    const xRatio = x / rect.width;
    // const yRatio = y / rect.height;
    return Math.floor(21 + (noteRange * xRatio)); // 21 = A0
  }
