...
describe('Scheduler', () => {
  ...
  let mizzy: jest.Mocked<Mizzy>;

  beforeEach(() => {
    // Clear all mocks before each test
    jest.clearAllMocks();
    mizzy = new Mizzy() as jest.Mocked<Mizzy>;
  ...
  describe('MIDI Clock Handling', () => {
    it('should process MIDI clock messages when playing', () => {
      ...
      const clockHandler = (mizzy.onMessage as jest.Mock).mock.calls[0][0];
    ...
    it('should handle MIDI start message', () => {
      ...
      const controlHandler = (mizzy.onMessage as jest.Mock).mock.calls[1][0];
